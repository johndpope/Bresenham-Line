...
extension Autoencoder {
    ...
    func embedding(from dataset: (images: Tensor<Float>, labels: Tensor<Int32>), shouldSaveInput: Bool, elementCount: Int32, step: Int) -> (labels: Tensor<Int32>, tensor: [Float]) {
        ...
        if shouldSaveInput {
            ...
            MyAutoEncoder.plot(image: Autoencoder.reshape(image: inputImage, imageCountPerLine: imagesInLine), name: "input-\(step)")
        }
        let outputImage = Array(result.output.scalars[0..<imagesInLine * imagesInLine * size])
        MyAutoEncoder.plot(image: Autoencoder.reshape(image: outputImage, imageCountPerLine: imagesInLine), name: "output-\(step)")
...
class MyAutoEncoder{
    ...
    static func plot(image: [Float], name: String) {
        // Create figure
        let ax = plt.gca()
